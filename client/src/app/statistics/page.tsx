...
export default function StatisticsPage() {
  ...
  const loadStatistics = async () => {
    ...
    try {
      ...
      if (data.totalStudents === 0 || !hasSubjects) {
        ...
        const hardestCode =
          data.hardestSubject?.subjectCode ??
          data.subjectStats?.[0]?.subjectCode ??
          null;
  ...
  return (
    ...
    >
      ...
      {/* Error state */}
      {status === "error" && (
        <div className="flex h-96 items-center justify-center rounded-2xl border border-red-500/20 bg-red-500/5 p-8 text-center">
          <div className="max-w-md space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              We encountered an issue communicating with the database. Please
      ...
      {/* Empty state */}
      {status === "empty" && (
        <div className="flex h-96 flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-card p-8 text-center">
          <div className="max-w-md space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              To view analytics, you must first import semester results in PDF
              format.
            </p>
      ...
      {status === "success" && statistics && (
        <div className="animate-in space-y-6 fade-in duration-300">
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            <div className="flex flex-col justify-start space-y-4 rounded-xl border border-border bg-card p-5 lg:col-span-1">
              <div>
